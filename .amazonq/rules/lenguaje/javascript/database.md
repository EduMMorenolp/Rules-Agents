# AccessAI Backend - Database Rules
...
const UserModel = (sequelize) => {
    const User = sequelize.define('User', {
        ...
        firstName: {
            type: DataTypes.STRING,
            allowNull: true, // Nullable para empresas
            validate: { len: [1, 100] }
        },
        isDeleted: {
...
# 20250814100003-create-zones.cjs
# 20250814100004-create-devices.cjs
# 20250814100005-create-alerts.cjs  ← SIGUIENTE NÚMERO
```

### ✅ **Patrón de Numeración Obligatorio**
```javascript
// FORMATO: YYYYMMDDHHMMSS-descripcion.cjs
// EJEMPLO: 20250814100005-create-shares.cjs
...
20250814100003-create-zones.cjs
20250814100004-create-devices.cjs
20250814100005-create-alerts.cjs
...
await queryInterface.addIndex('reactions', ['post_id', 'user_id'], { unique: true });
    ...
    const { count, rows } = await Alert.findAndCountAll({
        ...
        include: [
            ...
            },
            {
                ...
                separate: true, // Evita N+1 queries
                where: { isDeleted: false },
                required: false
            }
        ],
...
module.exports = {
    async up(queryInterface, Sequelize) {
        ...
        const personaRole = roles.find(r => r.key === 'persona');
        const empresaRole = roles.find(r => r.key === 'empresa');
...
async register(userData) {
    ...
    try {
        ...
        // Crear perfil de negocio si es empresa
        if (userData.roleKey === 'empresa') {
...
Esta configuración garantiza desarrollo ágil y producción segura del sistema AccessAI.